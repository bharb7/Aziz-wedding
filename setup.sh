...
cat > package.json << 'BEITALWARD_EOF'
{
  "name": "beit-al-ward-seating",
  "version": "1.0.0",
  "description": "Live shared wedding seating chart for Harb's wedding at Beit Al Ward",
  "type": "module",
  "scripts": {
    "dev": "vite",
    "build": "vite build",
...
cat > vite.config.js << 'BEITALWARD_EOF'
...
cat > tailwind.config.js << 'BEITALWARD_EOF'
...
cat > postcss.config.js << 'BEITALWARD_EOF'
export default {
  plugins: {
    tailwindcss: {},
    autoprefixer: {},
...
cat > index.html << 'BEITALWARD_EOF'
<!doctype html>
<html lang="ar" dir="rtl">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>بيت الورد — لوحة الجلوس الحية</title>
...
cat > src/main.jsx << 'BEITALWARD_EOF'
...
cat > src/index.css << 'BEITALWARD_EOF'
@tailwind base;
@tailwind components;
@tailwind utilities;
...
cat > src/supabase.js << 'BEITALWARD_EOF'
// Supabase setup.
// 1. Go to https://supabase.com → New project (free tier is plenty for this)
// 2. Once created, go to Project Settings → API
//    Copy the "Project URL" and the "anon public" key, paste them below.
// 3. Go to the SQL Editor (left sidebar) and run this once to create the table:
//
//    create table seating_state (
//      id int primary key default 1,
//      guests jsonb not null,
//      updated_at timestamptz default now()
//    );
//    insert into seating_state (id, guests) values (1, '[]'::jsonb);
//    alter table seating_state enable row level security;
//    create policy "Public read" on seating_state for select using (true);
//    create policy "Public write" on seating_state for update using (true);
//
//    (Public read/write is fine for sharing privately with a handful of
//    family members. See the README for how to lock this down further.)
// 4. In the left sidebar: Database → Replication → toggle on
//    "seating_state" so realtime updates broadcast to every open tab.
...
cat > src/App.jsx << 'BEITALWARD_EOF'
...
echo "Next: npm install, then npm run dev to test, or push to GitHub."